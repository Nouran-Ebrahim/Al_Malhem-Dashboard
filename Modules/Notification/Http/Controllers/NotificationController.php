...
class NotificationController extends Controller
{
    ...
    public function store(NotificationRequest $request)
    {
        ...
        $data['title'] = ['en' => $request->get('title_ar'), 'ar' => $request->get('title_ar')];
        $data['description'] = ['en' => $request->get('description_ar'), 'ar' => $request->get('description_ar')];
        ...
        if (isset($data['all_clients'])) {
            ...
            $clients_tokens = Client::active()->whereNotNull('fcm_token')->pluck('fcm_token')->all();
            $fcm = new FCMService;
        ...
        //all Clients Checkbox is not checked
            ...
            $clients_tokens = Client::active()->whereNotNull('fcm_token')->whereIn('id', $data['clients'])->pluck('fcm_token')->all();
            if(count($clients_tokens) == 0)
            {
                return redirect()->back();
            }
            $fcm = new FCMService;
