...
@section('content')
        ...
        <div class="card">
            <div
                class="
                    ...
                    ">
                <div>
                    <h4 class="card-title mb-25"> فلترة الحفلات
                    </h4>
    ...
    <!--/ Basic table -->
    {{ $superior->withQueryString()->links() }}
        ...
        <div class="modal-dialog modal-dialog-centered">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="myModalLabel120"> تعديل التفوق </h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <form action="{{ url('admin/addParty') }}" method="POST" enctype="multipart/form-data">
                            {{ method_field('POST') }}
                            {{ csrf_field() }}
                            <input type="hidden" value="" id="superior_id" name="id">
                            <div class="col-12">
                                <div class="mb-1 row">
                                    <div class="col-sm-3 text-center">
                                        <label class="col-form-label" for="pass-icon"> اضافة حفله</label>
                                    </div>
                                    <div class="col-sm-9">
                                        <div class="input-group-merge">
                                            <select name="party_id" id="status"
                                                class="select2 form-select select2-hidden-accessible order_status"
                                                tabindex="-1" aria-hidden="true">
                                                <option value="{{ null }}">اختر الحفلة</option>
                                                @foreach ($viewModel->parties() as $party)
                                                    <option @if (in_array($party->id, $party_ids)) selected @endif
                                                        value="{{ $party->id }}">{{ $party->title }}</option>
                                                @endforeach
                                            </select>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-success" data-bs-dismiss="modal">اضافه</button>
                </div>
            </div>
            </form>
        </div>
...
@section('js')
    ...
    @if (session('updated'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم تعديل  التفوق بنجاح',
                icon: 'success',
                customClass: {
    ...
    @if (session('created'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم انشاء  التفوق بنجاح',
                icon: 'success',
                customClass: {
                    confirmButton: 'btn btn-primary'
                },
    ...
    @endif
    <script>
        function getsuperiorId(val, party_id) {

            var id = document.getElementById("superior_id").value = val;
            // console.log(val, party_id);
            // const phpecho="<?php echo $party; ?>";
            // if (party_id != null) {
            //   phpecho=party_id;
            // }
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                return (
                                    ...
                                    @can('Edit-superior')
                                        +
                                        '<a href="superior/' + data +
                                            '/edit" class="item-edit">' +
                                            feather.icons['edit'].toSvg({
                                                class: 'font-small-4'
                                            }) +
                                            '</a>' +
                                            '<a onclick="getsuperiorId(' + full['id'] +
                                            ',' + full['party_id'] +
                                            ')" href="#" class="item-edit" data-bs-toggle="modal" data-bs-target="#changeModal">' +
                        ...
                        @can('Create-superior')
...

                            {
                                ...
                                action: function(e, dt, node, config) {
                                    ...
                                    window.location.href = './superior/create';
                                },
                                init: function(api, node, config) {
                                    $(node).removeClass('btn-secondary');
                                }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: window.location.origin + "/admin/superior/" + id,
                            type: 'POST',
                            data: {
                                "id": id,
                                "_method": "DELETE",
