...
@section('content')
        ...
        <div class="card">
            <div
                class="
                    ...
                    ">
                <div>
                    <h4 class="card-title mb-25"> فلترة المناسبات
                    </h4>
                    ...
                    <form class="dt_adv_search">
                        <div class="row g-1 mb-md-1">
                            <div class="col-md-4">
                                ...
                                <select name="occasions_category_id" class="form-select" id="name522">
                                    <option selected value="{{ null }}">اختر</option>

                                    @foreach ($viewModel->OccasionsCategories() as $occasionCategory)
                                        <option @if ($occasionCategory->id == @$_GET['occasions_category_id']) selected @endif
                                            value="{{ $occasionCategory->id }}">{{ $occasionCategory->title }}
                                        </option>
                                    @endforeach
    ...
    <!--/ Basic table -->
        {{ $occasions->withQueryString()->links() }}
...
@section('js')
    ...
    @if (session('updated'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم تعديل  المناسبة بنجاح',
                icon: 'success',
                customClass: {
    ...
    @if (session('created'))
        <script>
            Swal.fire({
                title: 'أحسنت!',
                text: 'لقد تم انشاء  المناسبة بنجاح',
                icon: 'success',
                customClass: {
                    confirmButton: 'btn btn-primary'
                },
    ...
    <script>
        $(function() {
            ...
            var occasionsCategory = url.searchParams.get("occasions_category_id")

            var ajaxRequest = "occasions?";
            var page = url.searchParams.get("page")
            if (page != null) {
                ajaxRequest += "page=" + page + '&';
            }
            if (occasionsCategory != null) {
                ajaxRequest += "occasions_category_id=" + occasionsCategory + "&";
            }
            ...
            if (dt_basic_table.length) {
                var dt_basic = dt_basic_table.DataTable({
                    ...
                    columns: [{
                        ...
                        {
                            data: 'occasions_categories.title'
                        },
                        {
                            data: 'is_active'
                        },
                        ...
                        },
                        {
                            ...
                            render: function(data, type, full, meta) {
                                ...
                                return (
                                    ...
                                    @can('Edit-occasions')
                                        +
                                        '<a href="occasions/activate/' + data +
                                            '/" class="dropdown-item ">' +
                        ...
                        @can('Create-occasions')
...

                            {
                                ...
                                action: function(e, dt, node, config) {
                                    ...
                                    window.location.href = './occasions/create';
                                },
                                init: function(api, node, config) {
                                    $(node).removeClass('btn-secondary');
                                }
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: window.location.origin + "/admin/occasions/" + id,
                            type: 'POST',
                            data: {
                                "id": id,
                                "_method": "DELETE",
