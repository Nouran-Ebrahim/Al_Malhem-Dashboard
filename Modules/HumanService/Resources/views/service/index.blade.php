        ...
        <div class="card">
            <div
                class="
                    ...
                    ">
                <div>
                    <h4 class="card-title mb-25"> فلترة الخدمات الانسانيه
                    </h4>
                    ...
                    <form class="dt_adv_search">
                        <div class="row g-1 mb-md-1">
                            <div class="col-md-4">
                                ...
                                <select name="service_type_id" class="form-select" id="name522">
                                    <option selected value="{{ null }}">اختر</option>

                                    @foreach ($viewModel->serviceTypes() as $serviceType)
                                        <option @if ($serviceType->id == @$_GET['service_type_id']) selected @endif
                                            value="{{ $serviceType->id }}">{{ $serviceType->title }}
                                        </option>
                                    @endforeach
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="type" class="form-select" id="name522">
                                    <option selected value="{{ null }}">اختر</option>

                                    <option value="offer" @if (@$_GET['type'] === 'offer') selected @endif> عرض
                                    </option>
                                    <option value="request" @if (@$_GET['type'] === 'request') selected @endif>طلب</option>
...
@section('js')
    ...
    <script>
        $(function() {
            ...
            var serviceType = url.searchParams.get("service_type_id")
            var type = url.searchParams.get("type")
            var ajaxRequest = "service?";
            var page = url.searchParams.get("page")
            if (page != null) {
                ajaxRequest += "page=" + page + '&';
            }
            if (serviceType != null) {
                ajaxRequest += "service_type_id=" + serviceType + "&";
            }
            if (type != null) {
            ...
            $('.datatables-basic tbody').on('click', '.delete-record', function() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: window.location.origin + "/admin/service/" + id,
                            type: 'POST',
                            data: {
                                "id": id,
                                "_method": "DELETE",
